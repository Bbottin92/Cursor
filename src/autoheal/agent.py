 ...
 class AutohealAgent:
     def __init__(
         ...
         *,
         ...
         self.control_socket_path = control_socket_path or default_control_socket_path(state_dir)
     ...
     def _handle_control(self, req: ControlRequest) -> ControlResponse:
         ...
         try:
             if method == "incidents.list":
                 ...
                 return ControlResponse(id=req.id, ok=True, result=db.list_incidents(conn, limit=limit))
     ...
     def run_once(self) -> None:
         ...
         try:
             ...
             for inc in db.list_incidents(conn, limit=500):
                 ...
                 if fp and fp not in active_fps:
                     db.mark_incident_fixed(conn, str(inc["id"]), "No longer detected by checks")
         ...
         log.debug("cycle finished in %.2fs (%d findings)", time.time() - cycle_started, len(findings))
     ...
     def _remediate_finding(self, conn: Any, incident_id: str, finding: Finding) -> None:
         ...
         for plan in plans:
             ...
             if result.status == "failed":
                 db.set_incident_last_error(conn, incident_id, (result.stderr or result.summary or "")[:4000])
