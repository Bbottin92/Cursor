 ...
 from .db import connect, create_manual_incident, get_incident, list_actions_for_incident, list_incidents
 ...
 def main(argv: list[str] | None = None) -> int:
     ...
     agent_run = agent_sub.add_parser("run", parents=[common], help="Run forever (daemon mode)")
     agent_run.add_argument("--no-control", action="store_true", help="Disable control socket")
     agent_once = agent_sub.add_parser("once", parents=[common], help="Run a single cycle and exit")
     ...
     rep_p.add_argument("--details-json", default="{}", help='JSON object string, e.g. \'{"k":"v"}\'')
 
     cfg_p = sub.add_parser("config", parents=[common], help="Config utilities")
     cfg_sub = cfg_p.add_subparsers(dest="cfg_cmd", required=True)
     cfg_init = cfg_sub.add_parser("init", parents=[common], help="Write a default config.json")
     cfg_init.add_argument("--path", help="Where to write (defaults to --config or auto default)")
     ...
     if args.cmd == "agent" and args.agent_cmd == "run" and args.no_control:
     ...
     if args.cmd == "agent" and args.agent_cmd == "run":
         AutohealAgent(cfg=cfg, state_dir=state_dir, db_path=db_path, control_socket_path=control_sock).run_forever()
         return 0
     ...
     if args.cmd == "incidents" and args.inc_cmd == "show":
         iid = str(args.id)
         if control_sock.exists():
             resp = call_control(control_sock, method="incidents.get", params={"id": iid}, token=None)
             if resp.ok:
                 _pjson(resp.result)
                 return 0
         conn = connect(db_path)
     ...
     if args.cmd == "config" and args.cfg_cmd == "init":
         out_path = Path(args.path or args.config or "").expanduser() if (args.path or args.config) else None
